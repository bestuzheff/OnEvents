...
Во время сборки ссылки на новые адреса сокращаются через `clck.ru` и сохраняются в `data/url_cache.json`.
Если этот файл изменился после добавления события, включите его в Pull Request. Повторные сборки используют сохранённую
короткую ссылку и не обращаются к сервису заново.
...
- [ ] Изменения `data/url_cache.json`, созданные при сборке, добавлены в PR;
...
- `data/url_cache.json` — кеш коротких ссылок на адреса мероприятий;
