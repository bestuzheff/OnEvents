...
Теперь есть два способа добавить фото:

**Способ 1 (Рекомендуемый): через файл `speakers.yml`**
1. Создайте файл `speakers.yml` в корне проекта (если нет).
2. Добавьте запись:
```yaml
"Имя Фамилия":
  avatar: "filename.jpg"
```
3. Положите файл с изображением в папку `img/`.

**Способ 2: Автоматически по имени**
1. Назовите файл именем спикера в транслите (slug), например `ivan-ivanov.jpg`.
2. Положите в `img/`.
