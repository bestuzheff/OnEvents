...
├── pyproject.toml             # Настройки линтера и форматтера Ruff
├── Dockerfile                 # Docker конфигурация
│
├── utils/                     # Универсальные утилиты
│   ├── text.py                # Работа с текстом (очистка, слаг, время)
│   ├── dates.py               # Работа с датами (склонение, интервалы)
│   └── url.py                 # Сокращение ссылок, UTM-метки, карты, часовые пояса
│
├── ics_calendars/             # Генерация ICS календарей
│   ├── vevents.py             # Создание VEVENT событий
│   └── generators.py          # Публичные календари, файлы для скачивания
│
├── webhtml/                   # Генерация HTML
│   ├── cards.py               # Карточки событий и вебинаров
│   └── calendars.py           # Блоки подписки на календари
│
├── rss/                       # RSS лента
│   └── rss.py                 # Генерация RSS 2.0
│
├── json_export/               # JSON экспорт
│   └── json.py                # Файлы для импорта
│
├── web/                       # HTML-шаблоны и Service Worker
│   ├── index.html             # Шаблон главной страницы
│   ├── video.html             # Шаблон страницы видеозаписей
│   ├── oneyear.html           # Шаблон страницы итогов года
│   └── sw.js                  # Шаблон Service Worker
│
├── data/                      # Кеш сокращённых ссылок
│   └── url_cache.json
│
├── events/                    # YAML файлы событий (входные данные)
├── webinars/                  # YAML файлы вебинаров (входные данные)
├── img/                       # Изображения (события, вебинары, города)
...
3. **Выход** — готовая статика в папке `site/` (HTML-страницы, ICS-календари, RSS, JSON, sitemap)
...
├── video/index.html        # Страница видеозаписей
├── oneyear/index.html      # Страница итогов года
├── sw.js                   # Service Worker (кеш статики)
├── sitemap.xml             # Карта сайта
├── robots.txt              # Правила для поисковых роботов
├── calendar/               # ICS календари для скачивания
│   ├── 2025-09-15-<название>.ics  # Индивидуальный календарь события
│   ├── onevents-public.ics        # Общий календарь (все события)
│   ├── onevents-public-moskva.ics # Календарь по городу
│   └── onevents-webinars.ics      # Календарь вебинаров
├── rss/
...
└── icons/                   # Иконки (копируются как есть)
```
...
```yaml
id: "xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx" # Уникальный идентификатор события
title: "Название события" # Название события
date: "2026-12-31" # Дата начала события, формат YYYY-MM-DD
...
| `test_oneyear_data.py` | Расчёт данных страницы итогов года: тепловая карта, разбивка по месяцам, география, граф связей, недавние события, облако слов, статистика |
| `test_create_web.py` | Полная сборка сайта, обработка невалидных YAML-файлов, sitemap, переключение темы |
| `test_service_worker.py` | Контракт кеширования Service Worker (через `tests/service_worker_harness.js`) |
