...
class TestCalendarGenerators(unittest.TestCase):
    ...
    def test_generate_public_calendars(self, mock_generate_public_calendar):
        ...
        with TemporaryDirectory() as temp_dir:
            ...
            self.assertTrue(
                (calendar_dir / "onevents-public.ics").exists()
            )
    ...
    @patch("ics_calendars.generators.generate_public_calendar")
    def test_generate_webinars_public_calendar(
        self,
        mock_generate_public_calendar
    ):
        ...
        with TemporaryDirectory() as temp_dir:
            ...
            result = generate_webinars_public_calendar(
                webinars,
                calendar_dir
            )
            ...
            self.assertEqual(
                result,
                "https://onevents.ru/calendar/onevents-webinars.ics"
            )

            webinars_file = (
                calendar_dir / "onevents-webinars.ics"
            )

            self.assertTrue(webinars_file.exists())

            self.assertEqual(
                webinars_file.read_text(encoding="utf-8"),
                "WEBINAR_ICS"
            )
    ...
    def test_generate_event_calendars(self, mock_generate_ics_content):
        ...
        with TemporaryDirectory() as temp_dir:
            ...
            self.assertEqual(
                mock_generate_ics_content.call_count,
                2
            )