...
class TestJsonExport(unittest.TestCase):
    def test_serialize_event_full(self):
        ...
        self.assertEqual(result['icon'], 'img/events/conf.png')
    ...
    def test_serialize_event_minimal(self):
        ...
        self.assertEqual(result['title'], 'Минимальное событие')
    ...
    def test_export_events_to_json(self):
        ...
        with TemporaryDirectory() as temp_dir:
            ...
            output_path = export_events_to_json(events, output_dir)

            self.assertTrue(output_path.exists())
            self.assertEqual(output_path.name, 'events.json')

            data = json.loads(output_path.read_text(encoding='utf-8'))

            self.assertEqual(len(data), 1)
            self.assertEqual(data[0]['title'], 'Событие')
    ...
    def test_export_upcoming_events_to_json(self):
        ...
        with TemporaryDirectory() as temp_dir:
            ...
            output_path = export_upcoming_events_to_json(events, output_dir)

            self.assertTrue(output_path.exists())
            self.assertEqual(output_path.name, 'events_upcoming.json')
    ...
    def test_export_webinars_to_json(self):
        ...
        with TemporaryDirectory() as temp_dir:
            ...
            output_path = export_webinars_to_json(webinars, output_dir)

            self.assertTrue(output_path.exists())
            self.assertEqual(output_path.name, 'webinars.json')

            data = json.loads(output_path.read_text(encoding='utf-8'))

            self.assertEqual(len(data), 1)

            self.assertEqual(data[0]['pic'], 'img/webinars/webinar.png')
    ...
    def test_export_upcoming_webinars_to_json(self):
        ...
        with TemporaryDirectory() as temp_dir:
            ...
            output_path = export_upcoming_webinars_to_json(webinars, output_dir)

            self.assertTrue(output_path.exists())

            self.assertEqual(output_path.name, 'webinars_upcoming.json')
